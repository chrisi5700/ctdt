...
{
    if constexpr (Var == DVar) return One<T>{}; // x / dx = 1
    if constexpr (Var != DVar) return Zero<T>{}; // x / dy = 0;
}
...
#define AddSimplification(cond, res)\
    else if constexpr (cond) {return typename argument_type<void(res)>::type{};}

#define EndBinaryOperatorSimplification(name)\
    else {return name<T, SE1, SE2>{};}\
}
